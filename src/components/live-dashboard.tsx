                        ...
                        <CardContent className="pt-6">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <div className="text-right">
                                    {node.online && (
                                        <>
                                            <div className="text-3xl font-bold">{node.cpu?.toFixed(0) || 0}%</div>
                                            <div className="text-xs text-muted-foreground uppercase">CPU Load</div>
                                        </>
                                    )}
                                </div>
                            </div>
                            ...
                            {node.online ? (
                                ...
                                <div className="h-8 flex items-center text-sm text-red-400 bg-red-50 rounded px-2">
                                    <Info className="h-4 w-4 mr-2" /> Connection Lost
                                </div>
                            )}
                        </CardContent>
                    </Card>
             ...
             <div className="md:col-span-1 space-y-4">
                 <Card>
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground">Across {plexNodes.length} servers</p>
                    </CardContent>
                 </Card>
                 <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{(totalBandwidth / 1000).toFixed(1)} Mbps</div>
                        <p className="text-xs text-muted-foreground">Upload Usage</p>
                    </CardContent>
                 </Card>
             </div>
                    ...
                    {allSessions.length === 0 ? (
                        ...
                        <div className="space-y-6">
                            {allSessions.map((session, i) => (
                                <div key={session.session_id + i} className="flex items-center justify-between border-b pb-4 last:border-0 last:pb-0">
                                    <div className="flex items-center space-x-4">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-sm text-muted-foreground">
                                                Watching <strong>{session.full_title || session.title}</strong>
                                            </p>
                                            <div className="flex items-center text-xs text-muted-foreground space-x-2">
                                                <Badge variant="outline" className="text-xs">{session.serverName}</Badge>
                                                <span>{session.quality_profile}</span>
                                                <span>•</span>
                                    ...
                                    <div className="text-right text-xs text-muted-foreground hidden md:block">
                                        <div>{session.transcode_decision === "direct play" ? "Direct Play" : "Transcoding"}</div>
                                        <div>{session.progress_percent}% Complete</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="space-y-5">
                        {services.length === 0 ? (
                            <div className="text-center py-8 text-muted-foreground text-sm">
                                ...
                                <p className="text-xs mt-1">Go to Settings to add apps.</p>
                            </div>
                        ) : (
                            services.map((svc) => (
                                <div key={svc.id} className="flex items-center justify-between group">
                                    <div className="flex items-center gap-3">
                                        <div className={`p-2 rounded-full ${svc.online ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}>
